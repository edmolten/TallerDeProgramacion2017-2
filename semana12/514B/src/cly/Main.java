...
public class Main {
    ...
    static class Stormtrooper extends point{
        Stormtrooper(double _x, double _y){super(_x,_y);}
        boolean alive = true;
    }
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        String[] nxy = input.split(" ");
        int n = Integer.valueOf(nxy[0]);
        int x = Integer.valueOf(nxy[1]);
        int y = Integer.valueOf(nxy[2]);
        int shots = 0;
        point hanSolo = new point(x,y);
        ArrayList<Stormtrooper> troopers= new ArrayList<>();
        String[] stormPos;

        for(int i = 0; i< n; i++){
            input = reader.nextLine();
            stormPos = input.split(" ");
            x = Integer.valueOf(stormPos[0]);
            y = Integer.valueOf(stormPos[1]);
            troopers.add(new Stormtrooper(x,y));
        }

        for(Stormtrooper trooper:troopers){
            if(trooper.alive) {
                for (Stormtrooper trooper2 : troopers) {
                    if (collinear(hanSolo, trooper, trooper2)  && trooper2.alive && trooper!=trooper2 ) {
                        trooper2.alive = false;
                    }
                }
                shots++;
                trooper.alive = false;
